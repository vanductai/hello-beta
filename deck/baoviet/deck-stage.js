...
(() => {
  ...
  class DeckStage extends HTMLElement {
    ...
    _enableRail() {
      ...
      // only a stored '1' (from the S key or TweaksPanel toggle) shows it.
    ...
    _render() {
      ...
      overlay.innerHTML = `
        ...
        <button class="btn sidebar" type="button" aria-label="Toggle sidebar" title="Toggle sidebar (S)">Sidebar<span class="kbd">S</span></button>
      `;

      overlay.querySelector('.prev').addEventListener('click', () => this._advance(-1, 'click'));
      overlay.querySelector('.next').addEventListener('click', () => this._advance(1, 'click'));
      overlay.querySelector('.reset').addEventListener('click', () => this._go(0, 'click'));
      overlay.querySelector('.sidebar').addEventListener('click', () => this._toggleRailKey());
    ...
    /** Toggle the thumbnail rail via the S keyboard shortcut or the
     *  overlay Sidebar button. Reuses the same animated show/hide path
     *  as the TweaksPanel's __deck_rail_visible message. */
    _toggleRailKey() {
      if (!this._railEnabled || this.hasAttribute('no-rail')) return;
